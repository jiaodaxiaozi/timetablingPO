...
Graph::Graph(Network &n, uint r) {
	...
	int ideal_t = n.getRequest(r).ideal_dep;
	...
	{
		// if t is after the latest departure, then stop
		if (t > latestDep_v[path[0]])
			break;
		// add the node to the graph
		Node* start = new Node(path[0], t, State::Move);
...
void Graph::unroll(deque<Node*> &active, Network &network){
	...
	if (state == State::Stop){
		...
		if (latestDep_v[curr_pos] > curr_time + T_STOP){
			...
			{
				if ((var == State::Wait)
					&& latestDep_v[curr_pos] <= curr_time + T_STOP + T_STEP)
					continue;
				Node *newNode;
	...
	//				23) arrive and wait for departure
		...
		if (latestDep_v[curr_pos] > curr_time + T_STEP){
			...
			{
				if ((var == State::Wait)
					&& latestDep_v[curr_pos] <= curr_time + T_STEP + T_STEP)
					continue;
				Node *newNode;
				if (!nodes[curr_pos].empty() &&
	...
	// Scenario 2) move and stop if there is a stop
		...
		bool stopRequested = (find(stops.begin(), stops.end(), next_pos) != stops.end());
		bool finalStation = (next_pos == path[path.size() - 1]);
		// 2) move and stop if there is a stop
		if (stopRequested || finalStation)	{
			uint moving_time = network.getNetwork()[curr_pos][next_pos][Scenario::FullStop];
			// check if the time is not beyond the latest arrival
			if (finalStation || latestDep_v[next_pos] > curr_time + moving_time + T_STOP){
				Node *newNode;
...
////////////////////////////////////////////////////////////////////////////////
void Graph::computeSP(matd &costs, Network &n){
