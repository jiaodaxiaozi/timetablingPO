...
function [B_star,x_0] = GeneratePotentialFixings(l,u,V)
%initial values
...
while k < k_max && n_i == true;
    %Solve Restricted LP, x_i fractional
    x_i = RMLP(V_perturbed,l,u);
    %Un-perturbed solution used for strongbranching and finding target
    %value in applyfixings
    if i == 0;
        x_0 = x_i;
    end
...
%strongbranching.
if isempty(B_star) == 1;
    j_star = strongbranching(x_0,l,u,V);
    B_star = j_star;
end